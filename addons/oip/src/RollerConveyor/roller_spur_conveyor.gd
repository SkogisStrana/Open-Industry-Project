...
func _get_custom_preview_node() -> Node3D:
	...
	var preview_scene := load("res://addons/oip/parts/RollerSpurConveyor.tscn") as PackedScene
...
func _ensure_internal_nodes() -> void:
	if _roller_material == null:
		_roller_material = load("res://addons/oip/assets/3DModels/Materials/Metall2.tres").duplicate(true) as BaseMaterial3D
...
func _ensure_rollers_node() -> void:
	if not is_instance_valid(_rollers):
		...
		if not is_instance_valid(_rollers):
			...
			_rollers.roller_scene = load("res://addons/oip/src/RollerConveyor/Roller.tscn") as PackedScene
